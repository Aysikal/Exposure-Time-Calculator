...
# ---------------- CONFIG ----------------
FILTER_FOLDER = r"C:\Users\AYSAN\Desktop\project\INO\ETC\Data\Rezaei_Hossein_Atanaz_Kosar_2025_11_04\light\Aysan\high\hot pixels removed\97b-8\aligned\i\reduced"
...
# --- Your fixed annulus geometry ---
ANNULUS_FACTOR = 2.4     # r=5 → inner=12
ANNULUS_WIDTH  = 3.0     # r=5 → outer=15

# Instrument parameters (fixed)
GAIN = 1/45
READNOISE = 5.0
...
# ---------------- Airmass Exclusion ----------------
exclude_airmass_ranges = [(1.23 , 1.25), (1.266, 1.272)]  
...
# -----------------------------------------------------
#           BEST-RADIUS PHOTOMETRY FUNCTION
# -----------------------------------------------------
...
for sid, (x_ref, y_ref) in enumerate(refined):
    ...
    for idx, fname in enumerate(fits_files):
        ...
        mask = dist <= best_r
        flux = np.sum(data_frame[mask])
        mag_inst = -2.5*np.log10(flux) if flux > 0 else np.nan
        mags.append(mag_inst + 20)

    # ----------- Show all cutouts in n×n grid -----------
    N = len(cutout_images)
    ...
    for i, ax in enumerate(axes.flat):
        if i < N:
            ...
            circ = Circle((cx, cy), r, edgecolor='yellow', facecolor='none')
            ax.add_patch(circ)

            inner = r * ANNULUS_FACTOR
            outer = inner + ANNULUS_WIDTH
            ...
            ax.axis('off')
    plt.suptitle(f"Star {sid+1} cutouts with best radii")
    plt.tight_layout()
    plt.show()
    ...
    X_arr = np.array(airmasses)
    Y_arr = np.array(mags)
    mask = np.isfinite(X_arr) & np.isfinite(Y_arr)

    # Exclude specified airmass ranges
    for x_min, x_max in exclude_airmass_ranges:
    ...
    if np.count_nonzero(mask) > 2:
        ...
        plt.scatter(X_arr[mask], Y_arr[mask], color='blue', label='Used for fit')
        # Show excluded points
        excluded_mask = np.isfinite(X_arr) & np.isfinite(Y_arr) & ~mask
        if np.any(excluded_mask):
            plt.scatter(X_arr[excluded_mask], Y_arr[excluded_mask], color='red', label='Excluded')
        plt.plot(xfit, yfit, color='green', label='Linear fit')
