...
fits_folder = r"C:\Users\AYSAN\Desktop\project\INO\ETC\Data\Rezaei_Hossein_Atanaz_Kosar_2025_11_04\dark\dark_2s\high\high"
...
# ---------------------------
# PARALLEL WAVELET CLEANING
# ---------------------------
def _cycle_worker(args):
    img, wavelet, level, dx, dy = args
    ...
    return reco_unrolled[:img.shape[0], :img.shape[1]]


def cycle_spin_clean(image, wavelet, level, shifts):
    img = image.astype(np.float32)
    tasks = [(img, wavelet, level, dx, dy) for dx, dy in shifts]

    with ProcessPoolExecutor() as ex:
        results = list(ex.map(_cycle_worker, tasks))

    return np.mean(results, axis=0).astype(np.float32)


# ================================================================
# MAIN PROCESSING LOOP (WINDOWS SAFE)
# ================================================================
def main():
    ...
    for fname in os.listdir(fits_folder):
        ...
        for i in range(1, num_iterations + 1):
            ...
            # Replace hot pixels with local medians
            ...
            percent_rms_reduction = (
                100.0 * (1.0 - s_clean_whole['rms'] / orig_stats['rms'])
                if orig_stats['rms'] != 0 else 0.0
            )
        ...
        print(f"  ↪ Cleaned FITS saved to: {out_fits}")
        print(f"  ↪ Diagnostics CSV saved to: {diagnostics_csv}")
        print(f"  ↪ ZScale PNG saved to: {png_out}")
