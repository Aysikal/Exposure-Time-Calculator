...
image_files = [
    ...
    if os.path.isfile(os.path.join(folder_path, f)) and f.lower().endswith('.fit')
]
image_files.sort()
...
for idx, (ax, radial_profile) in enumerate(zip(axes, radial_profiles)):
    ...
    ax.axhline(y=half_max, color='r', linestyle='--')
...
bin_centers = (bin_edges[:-1] + bin_edges[1:]) / 2
ax.hist(FWHM_arcsec, bins=10, edgecolor='k', alpha=0.6, label='Data')
ax.set_xlabel('FWHM (arcseconds)', fontsize=12)
ax.set_ylabel('Density', fontsize=12)
ax.set_title('Histogram of FWHM values', fontsize=12)
ax.legend(fontsize=10)
median_fwhm = np.mean(FWHM_arcsec)
ax.text(0.6, 0.95, f'Median seeing for {filter} filter ({mode}): {median_fwhm:.2f} arcseconds',
