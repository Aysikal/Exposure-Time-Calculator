# extinction_pipeline_fixed_aperture_netpos_filter_POSITIVE_K_UTCcheck_filtered.py
...
# ---------------- CONFIG ----------------
ALIGNED_FOLDER = r"C:\Users\AYSAN\Desktop\project\INO\ETC\Outputs\reduced\Oct 1st, area 95 i high\keep\aligned"
...
AIRMMIN_CUTOFF = 1.22
  # <---- IGNORE points below this airmass
...
for sid, (x_ref, y_ref) in enumerate(refined):
    ...
    for idx, fname in enumerate(fits_files):
        ...
        if not np.isfinite(airmass_val) or airmass_val < AIRMMIN_CUTOFF:
            # skip invalid or below-threshold
            continue
    ...
    if not records:
        print(f"Star {sid+1}: no valid records after filtering airmass < {AIRMMIN_CUTOFF}")
        continue

    # ---- Sort + CSV ----
    records_sorted = sorted(records, key=lambda r: (r["dt_utc_obj"] if r["dt_utc_obj"] else r["time_utc"]))
    ...
    if np.count_nonzero(mask) > 2:
        ...
        ax2.set_title(f"Star {sid+1}: Extinction Fit (filtered X≥{AIRMMIN_CUTOFF})")
        ax2.legend()
        ...
        print(f"Star {sid+1}: not enough valid points for extinction fit")
...
if all_star_fits:
    ...
    ax3.set_title(f"All Stars: Extinction Fits Overlay (X≥{AIRMMIN_CUTOFF})")
